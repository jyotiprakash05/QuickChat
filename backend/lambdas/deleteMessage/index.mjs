...
export const handler = async (event) => {
  ...
  try {
    ...
    // Update the conversation's last message in CONVERSATIONS table
    // (Find the new most recent message)
    const recentMessagesResult = await docClient.send(new QueryCommand({
      TableName: TABLES.MESSAGES,
    ...
    const newLastMsg = recentMessagesResult.Items?.[0] || null;
      ...
      try {
        await docClient.send(new UpdateCommand({
          ...
          ExpressionAttributeValues: {
            ':msg': newLastMsg ? newLastMsg.content.substring(0, 100) : 'Message deleted',
            ':ts': newLastMsg ? newLastMsg.timestamp : new Date().toISOString(),
          },
        }));
      } catch (err) {
