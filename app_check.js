// ============================================
// QuickChat ΓÇö Main Application (AWS-Integrated)
// ============================================
...
async function loadStats() {
  ...
  try {
    ...
    if (badge) badge.textContent = `≡ƒæÑ ${state.totalUsers} users on QuickChat`;
...
function connectWebSocket() {
  ...
  WsService.on('newMessage', async (msg) => {
    ...
    if (conv) { 
      conv.lastMessage = msg.type === 'voice' ? '≡ƒÄÖ∩╕Å Voice message' : msg.content; 
...
async function sendVoiceMessage(blob, duration) {
  ...
  if (c) { c.lastMessage = '≡ƒÄÖ∩╕Å Voice message'; c.lastMessageAt = newMsg.timestamp; }
  state.conversations.sort((a,b) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt));
      ...
      <main class="landing-main">
        <section class="landing-hero">
          <div class="landing-hero__content">
            <div class="badge-pill" style="margin: 0 auto 24px; display: inline-flex;">Γ£¿ The new standard for communication</div>
      ...
      <div class="auth-split">
        <div class="auth-split__left">
          ...
          <div class="auth-features">
            <div class="auth-feature-item"><span class="feature-icon">ΓÜí</span> Lightning fast real-time sync</div>
            <div class="auth-feature-item"><span class="feature-icon">≡ƒöÆ</span> Secure cloud infrastructure</div>
            <div class="auth-feature-item"><span class="feature-icon">≡ƒÄ¿</span> Gorgeous, fluid user interface</div>
          </div>
          <div style="margin-top: auto; padding-top: 40px;">
            ${state.isLive ? '<span class="live-badge">ΓùÅ Live Production Environment</span>' : '<span class="demo-badge">Demo Mode</span>'}
          </div>
        </div>
...
function renderSidebar() {
  ...
  let convListHtml = '';
  if (!filtered.length) { convListHtml = `<div class="no-results">${state.searchQuery?'No conversations found':'No chats yet ΓÇö start a new one!'}</div>`; }
  else {
    convListHtml = filtered.map(conv => {
      ...
      // Show '≡ƒÄÖ∩╕Å Voice message' for voice messages instead of raw S3 URL
      let rawPreview = conv.lastMessage || '';
      if (!isTyping && rawPreview && extractS3Key(rawPreview)) rawPreview = '≡ƒÄÖ∩╕Å Voice message';
  ...
  sb.innerHTML = `
    ...
    ${state.isLive && state.totalUsers !== null
      ? `<div class="user-count-badge" id="user-count-badge">≡ƒæÑ ${state.totalUsers} users on QuickChat</div>`
      : ''}
      ...
      <div class="profile-body scrollbar-hide">
        ...
        <div class="profile-section">
          <div class="profile-card">
            ...
            <div class="profile-field"><span class="profile-field__icon">${state.wsConnected?ICONS.wifi:ICONS.wifiOff}</span><div class="profile-field__content"><p class="profile-field__label">Connection</p><p class="profile-field__value">${state.isLive?(state.wsConnected?'Live ΓÇö Connected':'Live ΓÇö Reconnecting...'):'Demo Mode'}</p></div></div>
          </div>
        </div>
        <div class="profile-section">
          <div class="profile-card">
  ...
  document.getElementById('btn-edit-profile').onclick = async () => {
    ...
    if (!editing) {
      ...
      if (newName) {
        ...
        document.getElementById('profile-save-msg').innerHTML = '<p class="profile-save-msg">Γ£ô Profile updated</p>';
